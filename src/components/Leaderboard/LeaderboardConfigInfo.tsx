...
export default function LeaderboardConfigInfo({
  xpType,
  xpRate,
  xpDecay,
  roleRewards,
}: LeaderboardConfigInfoProps) {
...
function XpTypeInfo({ xpType }: { xpType: LeaderboardConfigInfoProps["xpType"] }) {
  switch (xpType) {
    case "global":
      return (
        <>
          This server uses the <b>global leaderboard</b>. Members can get XP from any other server that also
          uses the global leaderboard.
        </>
      );
    case "local":
      return (
        <>
          This server uses the <b>local leaderboard</b>. Members can only get XP from this server, based on
          their chat activity.
        </>
      );
    case "mee6-like":
      return (
        <>
          This server uses the <b>mee6-like leaderboard</b>. Members can only get their XP from this server,
          based on the number of messages they send.
        </>
      );
    default:
      return (
        <>
          This server uses the <b>{xpType}</b> leaderboard.
        </>
      );
  }
}
...
function XpRateInfo({
  ...
  xpType: LeaderboardConfigInfoProps["xpType"];
  xpRate: LeaderboardConfigInfoProps["xpRate"];
}) {
  ...
  if (xpType === "mee6-like") {
    ...
    return (
      <>
        The XP rate is <b>x{formatedRate}</b>: each message sent will randomly reward between {minValue} and{" "}
        {maxValue} XP.
      </>
    );
    ...
    if (xpRate === 1) {
      ...
      return (
        <>
          The XP rate is <b>x{formatedRate}</b>: each message will reward less XP than the default value,
          based on its length.
        </>
      );
    }
...
function XpDecayInfo({ xpDecay }: { xpDecay: LeaderboardConfigInfoProps["xpDecay"] }) {
  if (xpDecay === 0) {
    return (
      <>
        The XP decay is
        <b>disabled</b>: members will not lose XP over time.
      </>
    );
  }
  ...
  return (
    <>
      The XP decay is set to
      <b>{formatedValue} XP</b>: each member will lose
      {formatedValue} XP every day, no matter their activity.
    </>
  );
}

function RoleRewardsInfo({ roleRewards }: { roleRewards: RoleReward[] }) {
  const sortedRoleRewars = roleRewards.toSorted((a, b) =>
      ...
      <List dense sx={{ listStyleType: "circle", py: 0 }}>
        {sortedRoleRewars.map((roleReward) => (
          <ListItem key={roleReward.roleId} sx={{ py: 0.2 }}>
            <RoleMention name={roleReward.role?.name ?? roleReward.roleId} color={roleReward.role?.color} />{" "}
            at level {formatNumber(roleReward.level)}
          </ListItem>
        ))}
      </List>
    </>
