    ...
    case "global":
      return (
        <>
          ...
          . Members can get XP from any other server that also uses the global leaderboard.
        </>
      );
    case "local":
      return (
        <>
          This server uses the
          <b>local leaderboard</b>
          . Members can only get XP from this server, based on their chat activity.
        </>
      );
    case "mee6-like":
      return (
        <>
          This server use the
          <b>mee6-like leaderboard</b>
          . Members can only get their XP from this server, based on the number of messages they send.
        </>
      );
    default:
      return (
  ...
  if (xpType === "mee6-like") {
    ...
    return (
      <>
        The XP rate is
        <b>x{formatedRate}</b>
        : each message sent will randomly reward between
        {minValue}
        {" "}
        and
        {maxValue}
    ...
    if (xpRate > 1) {
      return (
        <>
          The XP rate is
          <b>x{formatedRate}</b>
          : each message will reward more XP than the default value, based on its length.
        </>
      );
    } else {
      return (
        <>
          The XP rate is
          <b>x{formatedRate}</b>
          : each message will reward less XP than the default value, based on its length
        </>
      );
    }
  }
...
function XpDecayInfo({ xpDecay }: { xpDecay: LeaderboardConfigInfoProps["xpDecay"] }) {
  if (xpDecay === 0) {
    return (
      <>
        The XP decay is
        <b>disabled</b>
        : members will not lose XP over time.
      </>
    );
  }
  ...
  return (
    <>
      ...
      <b>{formatedValue} XP</b>
      : each member will lose
      {formatedValue}
      {" "}
      XP every day, no matter their activity.
    </>
  );
}
