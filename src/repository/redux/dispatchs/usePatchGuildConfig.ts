...
export function usePatchGuildConfig() {
  ...
  const [patchGuildConfigMutation, { error: errorOnGuildConfig, isLoading: isLoadingGuildConfig }] = usePatchGuildConfigMutation();
  const [putRoleRewardsMutation, { error: errorOnRoleRewards, isLoading: isLoadingRoleRewards }] = usePutGuildRoleRewardsMutation();
  ...
  async function putRoleRewardsCommand(guildId: string, roleRewards: Parameters<typeof putRoleRewardsMutation>[0]["roleRewards"]) {
    setSuccess(false);
